...
export default function AdminPanel({ user, logout }: AdminPanelProps) {
  ...
  return (
    <div className="flex-1 h-screen bg-[#070B13] flex flex-col font-sans text-text-primary overflow-hidden relative">
      {/* Glow Background */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 size-200 bg-accent-default/5 rounded-full blur-[160px] pointer-events-none" />

      {/* Top Navigation / Dashboard Header */}
      <header className="h-18 border-b border-border-divider bg-[#0A0F1D]/80 backdrop-blur-xl px-6 flex justify-between items-center z-10">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2.5 bg-secondary/30 border border-white/5 py-1.5 pl-2.5 pr-4 rounded-xl">
            {user.avatar_url ? (
              <Image
                src={user.avatar_url}
                alt={user.username}
          ...
          <button
            onClick={logout}
            className="w-10 h-10 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/10 hover:border-red-500/20 rounded-xl flexcc transition-all active:scale-95"
            title="Keluar"
          >
      ...
      {/* Tabs */}
      <div className="px-6 pt-6 flex gap-2 border-b border-border-divider bg-[#070B13] z-10">
        <button
          onClick={() => setActiveTab("users")}
          className={clsx(
            "px-5 py-3 text-sm font-bold border-b-2 transition-all flex items-center gap-2",
            activeTab === "users"
              ? "border-accent-default text-white"
      ...
      <div className="flex-1 overflow-y-auto p-6 z-10">
        {activeTab === "users" && (
          <div className="space-y-6">
            {/* Filters / Search Bar */}
            <div className="flex flex-col md:flex-row gap-3 bg-secondary/20 border border-white/5 p-4 rounded-2xl">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full bg-[#0A0F1D]/80 border border-border-divider rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-text-muted focus:border-accent-default focus:ring-1 focus:ring-accent-default focus:outline-none transition-all"
                />
              </div>
              ...
              <div className="flex gap-3">
                <select
                  ...
                  className="bg-[#0A0F1D]/80 border border-border-divider rounded-xl px-4 py-2.5 text-sm text-white focus:border-accent-default focus:outline-none transition-all"
                >
            ...
            <div className="bg-secondary/10 border border-white/5 rounded-2xl overflow-hidden shadow-xl">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse text-sm">
                  <thead>
                    <tr className="border-b border-border-divider bg-[#0A0F1D]/30 text-text-muted font-bold text-xs uppercase tracking-wider">
                          ...
                          <td className="py-4 px-6 text-right">
                            <div className="flex justify-end gap-2">
                              ...
                              <button
                                ...
                                className="px-3 py-1.5 bg-accent-default/10 hover:bg-accent-default/20 border border-accent-default/20 text-accent-default rounded-lg text-xs font-bold transition-all flex items-center gap-1"
                                title="Kirim Notifikasi Official"
                              >
              ...
              {pagination.total_pages > 1 && (
                <div className="px-6 py-4 border-t border-border-divider/50 bg-[#0A0F1D]/20 flex justify-between items-center text-xs text-text-muted">
                  <span>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="p-2 bg-secondary/30 hover:bg-secondary/50 rounded-lg border border-white/5 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                    >
        ...
        {activeTab === "moderators" && isSuperAdmin && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <form onSubmit={handleCreateInvite} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#0A0F1D]/80 border border-border-divider rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-text-muted focus:border-accent-default focus:ring-1 focus:ring-accent-default focus:outline-none transition-all"
                  />
                </div>
                ...
                <button
                  type="submit"
                  disabled={isInviting}
                  className="w-full py-3 bg-accent-default hover:bg-accent-hover text-white font-black rounded-xl text-sm transition-all active:scale-95 shadow-lg shadow-accent-default/10 flexcc gap-2 disabled:opacity-50"
                >
                  {isInviting ? (
                    <>
            ...
            <div className="lg:col-span-2 bg-secondary/10 border border-white/5 rounded-2xl overflow-hidden shadow-xl">
              <div className="px-6 py-4 border-b border-border-divider/50 bg-[#0A0F1D]/30 flex justify-between items-center">
                      ...
                      >
                        ...
                        {!invite.used_at && (
                          <button
                            ...
                            className="px-3 py-1.5 bg-secondary hover:bg-secondary/80 border border-white/5 text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1.5"
                          >
      ...
      {notifTargetUser && (
        <div className="fixed inset-0 bg-black/70 flexcc z-999 p-4 backdrop-blur-sm">
            ...
            <form onSubmit={handleSendNotification} className="space-y-4">
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full bg-[#0A0F1D] border border-border-divider rounded-xl px-4 py-3 text-sm text-white placeholder:text-text-muted focus:border-accent-default focus:outline-none transition-all resize-none"
                />
              </div>
              ...
              <div className="flex gap-2 justify-end">
                <button
                  ...
                  className="px-4 py-2 text-xs font-bold text-text-muted hover:text-white transition-all"
                >
                  Batal
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-accent-default hover:bg-accent-hover disabled:opacity-50 text-white text-xs font-black rounded-lg transition-all flex items-center gap-1"
                >
      ...
      <ConfirmModal
        ...
        description={`Lu yakin mau mengubah status akun @${targetUser?.username} menjadi ${pendingStatusChange}? User mungkin akan langsung kehilangan akses atau terputus secara real-time.`}
        confirmLabel={`Ya, Ubah`}
        cancelLabel="Batal"
        variant={pendingStatusChange === "active" ? "info" : "danger"}
      />
    </div>
  );
}
