...
export default function MessageBubbleMenu({ isMe, onInfoClick }: MessageBubbleMenuProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [menuCoords, setMenuCoords] = useState({ top: 0, left: 0, right: 0, bottom: 0, opensUp: false });
      ...
      {mounted && isOpen && createPortal(
        <>
          ...
          <div
            style={{
              position: 'fixed',
              ...(menuCoords.opensUp ? { bottom: menuCoords.bottom } : { top: menuCoords.top }),
              ...(isMe ? { right: menuCoords.right } : { left: menuCoords.left })
            }}
            className={clsx(
              "bg-elevated border border-border-divider rounded-xl shadow-2xl py-1 z-[9999] w-36 overflow-hidden",
              isMe ? (menuCoords.opensUp ? "origin-bottom-right" : "origin-top-right") : (menuCoords.opensUp ? "origin-bottom-left" : "origin-top-left")
            )}
          >
            {isMe && (
              <button
